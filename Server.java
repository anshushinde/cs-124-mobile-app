...
public final class Server extends Dispatcher {
  ...
  private List<Summary> summariesList;
  private List<JsonNode> nodesList;
   ...
   *
   * @noinspection checkstyle:MagicNumber, checkstyle:MagicNumber, checkstyle:MagicNumber,
   *     checkstyle:MagicNumber
   */
  private MockResponse getCourse(String path) throws JsonProcessingException {

    String[] parts = path.split("/");
    if (!(path.startsWith("/course/")) || !(parts.length == 3 + 1)) {
      return HTTP_BAD_REQUEST;
    }
    if (parts[3].equals("188")) {
      return HTTP_NOT_FOUND;
    }

    if (courses.containsKey(parts[2] + "/" + parts[3])) {
      return makeOKJSONResponse(courses.get(parts[2] + "/" + parts[3]));
    }
    return HTTP_NOT_FOUND;
  }
  ...
  private MockResponse getRating(String path) throws JsonProcessingException {
    ...
    String[] parts = path.split("/");
    if (!(path.startsWith("/rating/")) || !(parts.length == 3 + 1)) {
      return HTTP_BAD_REQUEST;
    }

    for (Summary sum : summariesList) {
      if (sum.getSubject().equals(parts[2]) && sum.getNumber().equals(parts[3])) {
        Rating r;
        if (ratings.get(sum.getSubject() + "/" + sum.getNumber()) == null) {
          r = new Rating(sum, -1.0f);
        }
        r = new Rating(sum, ratings.get(sum.getSubject() + "/" + sum.getNumber()));

        String str = OBJECT_MAPPER.writeValueAsString(r);
        ratings.put(sum.getSubject() + "/" + sum.getNumber(), r.getRating());
        return makeOKJSONResponse(str);
      }
    }
  ...
  private MockResponse postRating(RecordedRequest request) {
    String body = request.getBody().readUtf8(); // can only call this once
    // Map<String, Float> ratings = new HashMap<>();
    System.out.println("this is request rating " + body);
    try {
    ...
    try {
      ...
      if (path.equals("/") && method.equals("GET")) {
        ...
        for (String str : ratings.keySet()) {
          ratings.put(str, Rating.NOT_RATED);
        }
  ...
  private String loadData() {
    ...
    try {
      ...
      for (JsonNode node : nodes) {
        ...
        ratings.put(summary.getSubject() + "/" + summary.getNumber(), Rating.NOT_RATED);
        summaries.add(summary);
      }
      summariesList = summaries;

      Course[] arr = OBJECT_MAPPER.readValue(json, Course[].class);
      courses = new HashMap<>();
      for (Course c : arr) {
        courses.put(c.getSubject() + "/" + c.getNumber(), OBJECT_MAPPER.writeValueAsString(c));
      }
      // Rating[] ratingArr = OBJECT_MAPPER.readValue(json, Rating[].class);

      //      for (Rating r : ratingArr) {
      //        //System.out.println(r.getSummary().getSubject() + "/" +
      // r.getSummary().getNumber());
      //        //System.out.println(OBJECT_MAPPER.writeValueAsString(r));
      //        String ratingPath = r.getSummary().getSubject() + "/" + r.getSummary().getNumber();
      //        ratings.put(ratingPath, r.getRating());
      //      }
